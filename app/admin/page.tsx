      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex items-center justify-center p-6" dir="rtl">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl max-w-md w-full space-y-6 text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black py-3.5 rounded-2xl shadow-lg transition active:scale-[0.99]"
            >
              دخول Admin 🚀
            </button>
          </form>
          ...
          <div className="pt-2">
            <a href="/" className="text-slate-500 hover:text-slate-400 text-xs font-semibold transition">
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white">لوحة تحكم متجر سهيل ⚙️</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold px-4 py-3 rounded-2xl border border-red-500/25 transition text-sm flex items-center gap-2"
            >
              <span>🚪</span>
              <span>خروج</span>
            </button>
        ...
        <div className={`bg-slate-900/80 backdrop-blur-md p-6 md:p-8 rounded-3xl border shadow-xl transition ${editingId ? 'border-amber-500/50 ring-2 ring-amber-500/20' : 'border-slate-800'}`}>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              <span>{editingId ? `تعديل بيانات الجهاز (ID: ${editingId})` : 'إضافة جهاز جديد مع الذاكرة والبطارية'}</span>
            </h2>
            {editingId && (
              <button
                type="button"
                onClick={handleCancelEdit}
                className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold px-3 py-1.5 rounded-xl border border-slate-700 transition"
              >
                إلغاء التعديل ✕
              </button>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">اسم الجهاز (مثال: iPad A16)</label>
              <input
                type="text"
                placeholder="أدخل اسم الجهاز بوضوح..."
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3.5 text-white placeholder-slate-600 focus:outline-none focus:border-amber-500"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="مثال: 128GB أو 256GB"
                  value={storage}
            ...
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2">
                {editingId ? 'تغيير صورة الجهاز (اختياري)' : 'اختر صورة الجهاز من الكمبيوتر'}
              </label>
              <input
                type="file"
                accept="image/*"
                onChange={(e) => setImageFile(e.target.files ? e.target.files[0] : null)}
                className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-slate-400 file:ml-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-amber-500 file:text-slate-950 hover:file:bg-amber-400 transition cursor-pointer"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full font-black py-4 rounded-2xl shadow-lg transition active:scale-[0.99] disabled:opacity-50 text-base ${
                editingId 
        ...
        <div className="bg-slate-900/80 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-slate-800 shadow-xl">
          ...
          <div className="space-y-4">
              ...
              <div key={item.id} className="flex items-center justify-between bg-slate-950 p-4 rounded-2xl border border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 font-bold px-4 py-2 rounded-xl border border-amber-500/20 text-sm transition"
                  >
                    تعديل ✏️
                  </button>
                  <button
                    onClick={() => handleDelete(item.id)}
                    className="bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold px-4 py-2 rounded-xl border border-red-500/20 text-sm transition"
                  >
                    حذف 🗑️
                  </button>
                </div>
            ...
            {products.length === 0 && (
              <p className="text-center text-slate-500 py-8">لا توجد أجهزة مسجلة حتى الآن.</p>
            )}
          </div>
        </div>
